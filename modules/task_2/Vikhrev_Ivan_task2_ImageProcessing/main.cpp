...
class img
{
    public:
		...
		img(const img &m);
		~img();
		bool loadImg(String name);
		bool saveImg(String name);
...
bool img::saveImg(String name)
{
	return imwrite(name, pic);
}
...
void img::createImg(unsigned int height, unsigned int width)
{
	...
	imgSize = cols*rows;
	type = CV_8U;
	pic = Mat(rows, cols, CV_8U, cv::Scalar(0));
	r.fill(pic, RNG::UNIFORM, cv::Scalar(0), cv::Scalar(255));
}

void applyFilter1(Mat& img, int kernel_size)
{
	medianBlur(img, img, kernel_size);
}
void applyFilter2(Mat& img, int kernel_size)
{
	blur(img, img, Size(kernel_size, kernel_size));
}
void applyFilter3(Mat& img, int kernel_size)
{
	GaussianBlur(img, img, Size(kernel_size, kernel_size), 0);
}
void img::show(String wName)
{
...
int main(int argc, char** argv)
{
	...
	MPI_Scatterv(origImg.pic.data, scount, dis, MPI_UNSIGNED_CHAR, dataIN, scount[rank], MPI_UNSIGNED_CHAR, MainProc, MPI_COMM_WORLD);// MPI_UNSIGNED_CHAR, data, portion, MPI_UNSIGNED_CHAR, MainProc, MPI_COMM_WORLD)

	Mat tmp(scount[rank] / cols, cols, CV_8U, dataIN);
	...
	if (rank == MainProc) p = tmp.data;
	MPI_Gather(p, portion, MPI_UNSIGNED_CHAR, dataOUT, portion, MPI_UNSIGNED_CHAR, MainProc, MPI_COMM_WORLD);

    if (rank == MainProc)
	{
		t2 = MPI_Wtime();
		procImgP = img(Mat(origImg.rows, origImg.cols, CV_8U, dataOUT));
		std::cout << "Parallel Time:   " << t2 - t1 << std::endl;
		
		if (parser.get<bool>("s"))
		{	
	
			origImg.show((String)"Original");
			procImgS.show((String)"Sequnce processed");
